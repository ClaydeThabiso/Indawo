  ...
  const handleSubmit = async (e) => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          let finalImageUrl =
            "https://images.unsplash.com/photo-1534723452862-4c874018d66d?auto=format&fit=crop&w=400"; // Fallback image
          ...
          await axios.post(
            "http://localhost:5000/api/business/addBusiness",
            payload,
            {
  ...
  return (
    <div className="flex min-h-screen flex-col justify-center px-6 py-12 lg:px-8 bg-slate-50">
      <div className="w-full max-w-md mx-auto mb-2">
        ...
        >
          <span className="mr-2 transform group-hover:-translate-x-1 transition-transform  lg:justify-start">
            ←
          </span>
          Back
        </button>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md bg-white p-8 shadow-sm rounded-xl border border-gray-100">
        {status === "success" && (
          <div className="mb-6 bg-gradient-to-r from-[#1D4A79] to-blue-800 p-6 rounded-xl text-center shadow-lg border-2 border-[#FDBA31]">
            ...
            <h3 className="text-[#FDBA31] font-black text-xl mb-1">
              Location Secured!
            </h3>
            <p className="text-white text-sm font-medium">
              You just mapped a new business and earned{" "}
              <span className="font-black text-[#FDBA31] text-lg bg-white bg-opacity-20 px-2 py-0.5 rounded">
                +50 Points
              </span>
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-bold text-[#1D4A79] mb-2">
              Shop Photo
            </label>
            <div className="mt-1 flex justify-center rounded-lg border border-dashed border-gray-400 px-6 py-8 hover:bg-gray-50 transition-colors relative overflow-hidden">
              ...
              <div className="text-center relative z-10">
                ...
                <div className="mt-4 flex text-sm leading-6 text-gray-600 justify-center">
                  ...
                  >
                    <span>
                      {imagePreview ? "Change Photo" : "Upload a file"}
                    </span>
                    <input
                      id="file-upload"
                      name="file-upload"
                ...
                <p className="text-xs leading-5 text-gray-500 mt-2 font-bold bg-white/80 px-2 rounded-full inline-block">
          ...
          <div>
            <label className="block text-sm font-bold text-[#1D4A79]">
              Business Name
            </label>
            <input
              type="text"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, address_name: e.target.value })
              }
              disabled={status !== "idle" && status !== "error"}
            />
          </div>
